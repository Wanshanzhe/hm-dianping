...
public class VoucherOrderServiceImpl extends ServiceImpl<VoucherOrderMapper, VoucherOrder> implements IVoucherOrderService {
    ...
    /**
     * 阻塞队列
     * 当一个线程尝试从队列中获取元素，如果他没有元素，那么这个线程就会被阻塞，直到队列中有元素，它才会被唤醒，并且获取元素
     */
        ...
        private void handlePendingList() {
            while (true) {
                try {
                    ...
                    // 2.判断消息获取是否成功
                    if (list == null || list.isEmpty()) {
                        // 2.1.如果获取失败，说明pending-list没消息，结束循环
                        break;
                    }
                    // 3.解析消息中的订单消息
                    MapRecord<String, Object, Object> record = list.get(0);
...
//    }
    ...
    public void createVoucherOrder(VoucherOrder voucherOrder) {
        ...
        //6. 扣减库存
        boolean success = seckillVoucherService.update().setSql("stock = stock - 1").eq("voucher_id", voucherId).gt("stock", 0) //保证库存大于0即可
                .update();
        if (!success) {
            //扣减失败
            log.error("库存不足！");
